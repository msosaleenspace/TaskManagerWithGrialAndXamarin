...
namespace Eleos3
{
    public abstract class ChartViewAdapter : ChartView 
    {
        ...
        public static readonly BindableProperty DefaultValueLabelColorProperty =
            BindableProperty.Create(
                nameof(DefaultValueLabelColor),
                typeof(Color),
                typeof(ChartViewAdapter),
                Color.Default,
                propertyChanged: DefaultValueLabelColorChanged);

        public Color DefaultValueLabelColor
        {
            get { return (Color)GetValue(DefaultValueLabelColorProperty); }
            set { SetValue(DefaultValueLabelColorProperty, value); }
        }

        private static readonly ColorTypeConverter FormsConverter = new ColorTypeConverter();

        private bool _parentSet;
        private SKColor? _defaultEntrySKColor, _defaultEntryTextSKColor, _defaultValueLabelColor;
        ...
        private static void DefaultValueLabelColorChanged(BindableObject bindable, object oldValue, object newValue)
        {
            var adapter = (ChartViewAdapter)bindable;

            adapter._defaultValueLabelColor = adapter.DefaultValueLabelColor != Color.Default ?
                SKColor.Parse(adapter.DefaultValueLabelColor.ToHexString()) :
                (SKColor?)null;
        }
        ...
        private Microcharts.ChartEntry CreateFromType(object elem)
        {
            ...
            bool textColorSet = false;
            SKColor textColor = default(SKColor);
            bool valueLabelColorSet = false;
            SKColor valueLabelColor = default(SKColor);
            ...
            foreach (var prop in type.GetProperties())
            {
                ...
                else if (string.Equals(prop.Name, "ValueLabelColor", StringComparison.InvariantCultureIgnoreCase))
                {
                    valueLabelColorSet = ConvertColor(prop.GetValue(elem), out valueLabelColor);
                }
            }
            ...
            if (valueLabelColorSet)
            {
                result.ValueLabelColor = valueLabelColor;
            }
            else if (_defaultValueLabelColor != null)
            {
                result.ValueLabelColor = _defaultValueLabelColor.Value;
            }
